...
def colocar(opc1,opc2):
    ...
    if opc2==1:
        ...
        if opc1==1:
            ...
            etiqueta1.place(x=400,y=80)
            etiqueta2.config(text="Nombre:")
            etiqueta2.place(x=400,y=120)
            etiqueta3.config(text="Tipo 1:")
            etiqueta3.place(x=400,y=160)
            etiqueta4.config(text="Tipo 2:")
            etiqueta4.place(x=400,y=200)
            etiqueta5.config(text="Habilidad:")
            etiqueta5.place(x=400,y=240)
            etiqueta6.config(text="Generacion:")
            etiqueta6.place(x=400,y=280)
            etiqueta7.config(text="Region:")
            etiqueta7.place(x=400,y=320)
            etiqueta8.config(text="Dummy:")
            etiqueta8.place(x=400,y=360)

            caja1.place(x=480,y=80)
            caja2.place(x=480,y=120)
            caja3.place(x=480,y=160)
            caja4.place(x=480,y=200)
            caja5.place(x=480,y=240)
            caja6.place(x=480,y=280)
            caja7.place(x=480,y=320)

            boton2.place(x=400,y=440)
        ...
        #Region
            ...
            caja2.place(x=480,y=80)
            caja1.place(x=480,y=120)
    ...
    #Reporte
        ...
        #Pokemon
        if opc1==1:
            etiqueta1.config(text="Dummy:")
            etiqueta1.place(x=400,y=80)

            boton6.place(x=400,y=440)
...
def agregar():
    ...
    if opcion1.get()==1:
        ...
        if var==True and var2==True and varNum==False:
            ...
            etiqueta8.place(x=400,y=870)
        
        #Caso contrario, no se agrega nada y se pierden los datos
        else:
            etiqueta8.config(text='ERROR: Generacion/Region inexistente o ID de pokemon ya existe')
            etiqueta8.place(x=400,y=870)
            print('ERROR: Generacion/Region inexistente o ID de pokemon ya existe')
    ...
    #Id
        ...
        if nodoObj != None: #Se checa que el id está en el árbol id
            if arbolPokedex.find(pk.getId()) == None: #Se checa que no esté repetido ya el id entre los pokedex
                ...
                pk.setGeneracion(input("Introduce la generacion a la que pertenece: ")) #Se añade la generacion
                for i in range(0,generacion.tamaño,1):
                ...
                if var == True:
                    var = False
                    pk.setRegion(input("Introduce el nombre de la region: "))#Se añade la region
                    for i in range(0,region.tamaño,1):
                    ...
                    if var == True:
                        ...
                        while True:
                            print("Menú: Introducir Pokemon a Pokedex %s" % pk.getNombre())
                            ...
                            if op == 1:#Se añaden los objetos de pokemon en ese arbol dentro de pk
                                ...
                                if arbolPokemon.find(ip) != None:
                                    ...
                                    print(fp.objeto.getNombre())
                                    pk.appendPokemones(fp.dato,fp)
                                else:
                                    print("Pokemon no existe")
                            elif op == 2:
                                break
                            else:
                        ...
                        print("Region inexistente")
                else:
                    print("Generacion inexistente")
            else:
                print("Id ya utilizada")
        else:
            print("Id no existe")
    ...
    #Region
        ...
        for i in range(0,region.tamaño,1):
            if r==region.get(i).dato.getNombre():
                var=True
                print(region.remove(i).dato.getNombre())
                #region.remove(i)
                break
    ...
    #Generacion
        ...
        for i in range(0,region.tamaño,1):
            if g==generacion.get(i).dato.getNombre():
                var=True
                print(generacion.remove(i).dato.getNombre())
                break
...
def consultar():
    ...
    #Pokedex
        ...
        #Checo que exista en el arbol
        if arbolPokedex.find(d)!=None:
            print("Id:",arbolPokedex.find(d).objeto.getId())
            ...
            while True:
                print("Seleccione la manera de presentar la lista de pokemon.")
                print("1- Preorden\n2-Inorden\n3-Posorden")
                op = int(input("Opcion:"))
                ...
                if op ==1: #Devuelve todos los pokemones dentro de del subArbol
                    for i in range(0,len(subArbolPokemon.preorden()),1):
                        print("#:",subArbolPokemon.preorden()[i].dato)
                    ...
                    for i in range(0,len(subArbolPokemon.inorden()),1):
                        print("#:",subArbolPokemon.inorden()[i].dato)
                    ...
                    for i in range(0,len(subArbolPokemon.posorden()),1):
                        print("#:",subArbolPokemon.posorden()[i].dato)
...
def modificar():
    ...
    #Generacion
        ...
        for i in range(0,generacion.tamaño,1):
            if g==generacion.get(i).dato.getNombre():
                #Actualización de datos
                nuevo=Generacion()
                nuevo.setConsola(caja3.get())
                nuevo.setNombre(caja4.get())
    ...
    #Pokedex
        ...
        var = False
        if arbolPokedex.find(d)!=None:
            g = input("Introduce la generacion a la que pertenece: ")
            for i in range(0,generacion.tamaño,1):
                if g==generacion.get(i).dato.getNombre():
                    var=True
                    break
            if var == True:
                ...
                if var == True:
                    ...
                    while True:
                        print("Menú: Introducir Pokemon a Pokedex %s" % arbolPokedex.find(d).objeto.getNombre())
                        print("1- Introducir Pokemon")
                        print("2- Finalizar")
                        op = int(input("Opcion: "))
                        if op == 1:
...
#fondo
bg = PhotoImage(file='Grafico\poked.png')
my_label = Label(ventana, image=bg)
my_label.place(x=0, y=0, relwidth=1, relheight=1)
ventana.resizable(False, False)
#Etiquetas #listo
etiqueta1 = Label(ventana,text="Numero:")  #x=400,y=80 #, bg="white"
etiqueta1.place(x=200,y=550)
etiqueta2 = Label(ventana,text="Nombre:") #x=400,y=120
etiqueta2.place(x=200,y=590)
etiqueta3 = Label(ventana,text="Tipo 1:")  #x400,y=160
etiqueta3.place(x=200,y=630)
etiqueta4 = Label(ventana,text="Tipo 2:") #x=400,y=200
etiqueta4.place(x=200,y=670)
etiqueta5 = Label(ventana,text="Habilidad:") #x=400,y=240
etiqueta5.place(x=200,y=710)
etiqueta6 = Label(ventana,text="Generacion:") #x=400,y=280
etiqueta6.place(x=200,y=750)
etiqueta7 = Label(ventana,text="Region:") #x=400,y=320
etiqueta7.place(x=200,y=790)
etiqueta8 = Label(ventana,text="Dummy:")
etiqueta8.place(x=200,y=830)


#Cajas #listo
caja1=Entry(ventana,textvariable=num1)    #x=480,y=80
caja1.place(x=345,y=550)
caja2=Entry(ventana,textvariable=cadena1) #x=480,y=120
caja2.place(x=345,y=590)
caja3=Entry(ventana,textvariable=cadena2) #x=480,y=160
caja3.place(x=345,y=630)
caja4=Entry(ventana,textvariable=cadena3) #x=480,y=200
caja4.place(x=345,y=670)
caja5=Entry(ventana,textvariable=cadena4) #x=480,y=240
caja5.place(x=345,y=710)
caja6=Entry(ventana,textvariable=cadena5) #x=480,y=280
caja6.place(x=345,y=750)
caja7=Entry(ventana,textvariable=cadena6) #x=480,y=320
caja7.place(x=345,y=790)
...
boton2.place(x=200,y=870)
boton3=Button(ventana,text="Dar de baja",command=baja)
boton3.place(x=200,y=870)
boton4=Button(ventana,text="Consultar",command=consultar)
boton4.place(x=200,y=870)
boton5=Button(ventana,text="Modificar",command=modificar)
boton5.place(x=200,y=870)
boton6=Button(ventana,text="Generar reporte",command=reporte)
boton6.place(x=200,y=870)

#RadioButton
#Clase #listo
radio1=Radiobutton(ventana,text="Pokemon",value=1,variable=opcion1)
radio1.place(x=200,y=80)
radio2=Radiobutton(ventana,text="Region",value=2,variable=opcion1)
radio2.place(x=200,y=120)
radio3=Radiobutton(ventana,text="Generacion",value=3,variable=opcion1)
radio3.place(x=200,y=160)
radio4=Radiobutton(ventana,text="ID",value=4,variable=opcion1)
radio4.place(x=200,y=200)
radioF=Radiobutton(ventana,text="Pokedex",value=5,variable=opcion1)
radioF.place(x=200,y=240)

#Accion #texto #listo
radio5=Radiobutton(ventana,text="Agregar",value=1,variable=opcion2)
radio5.place(x=350,y=80)
radio6=Radiobutton(ventana,text="Dar de baja",value=2,variable=opcion2)
radio6.place(x=350,y=120)
radio7=Radiobutton(ventana,text="Consultar",value=3,variable=opcion2)
radio7.place(x=350,y=160)
radio8=Radiobutton(ventana,text="Modificar",value=4,variable=opcion2)
radio8.place(x=350,y=200)
radio8=Radiobutton(ventana,text="Generar reporte",value=5,variable=opcion2)
